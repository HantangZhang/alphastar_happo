...
from baiduagent.baidu.algorithms.utils.util import check, init
from baiduagent.baidu.algorithms.utils.transformer_act import discrete_autoregreesive_act
from baiduagent.baidu.algorithms.utils.transformer_act import discrete_parallel_act
from baiduagent.baidu.algorithms.utils.transformer_act import continuous_autoregreesive_act
from baiduagent.baidu.algorithms.utils.transformer_act import continuous_parallel_act
...
class SelfAttention(nn.Module):
    ...
    def forward(self, key, value, query):
        B, L, D = query.size() #B=1,L=5,D=64,表示
        #q,k,v均为[1,5,64]的矩阵,与
        ...
        att = F.softmax(att, dim=-1) #att = [1,1只有一个头,5,5]
...
class DecodeBlock(nn.Module):
    ...
    def forward(self, x, rep_enc):
        x = self.ln1(x + self.attn1(x, x, x)) #x从[1,5,19]编码成[1,5,64],rep_enc是encoder的输出
...
class Decoder(nn.Module):
    ...
    def forward(self, action, obs_rep, obs):
        ...
        if self.dec_actor:  #action_dim = [1,5,19]
            if self.share_actor:
                logit = self.mlp(obs)
            else:
                logit = []
            ...
            action_embeddings = self.action_encoder(action) #[1,5,64]
            x = self.ln(action_embeddings) #[1,5,64]
...
class MultiAgentTransformer(nn.Module):
    ...
    def get_actions(self, state, obs, available_actions=None, deterministic=False):
        ...
        state = np.zeros((*ori_shape[:-1], 37), dtype=np.float32) #state.shape = [1,5,37]

        state = check(state).to(**self.tpdv) #将state存储到当前的设备上,gpu或cpu
        obs = check(obs).to(**self.tpdv)
        if available_actions is not None:
            available_actions = check(available_actions).to(**self.tpdv)
