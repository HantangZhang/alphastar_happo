...
def discrete_autoregreesive_act(decoder, obs_rep, obs, batch_size, n_agent, action_dim, tpdv,
    ...
    for i in range(n_agent):
        ...
        if available_actions is not None:
            logit[available_actions[:, i, :] == 0] = -1e10 #进行非法动作mask

        distri = Categorical(logits=logit)
        action = distri.probs.argmax(dim=-1) if deterministic else distri.sample() #从1,18中选择得分最高的一维作为该agent的动作
        action_log = distri.log_prob(action) #取action对应概率的对数

        output_action[:, i, :] = action.unsqueeze(-1) #将动作赋值给对应的agent
        output_action_log[:, i, :] = action_log.unsqueeze(-1) #将动作概率对数赋值给对应的agent
        if i + 1 < n_agent:
