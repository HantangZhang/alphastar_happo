...
import subprocess
import logging
from agent_config import RTMNum
import os
import re
import time
...
from agent_config import HostID
class XSimManager(object):
    ...
    def __start_env(self):
        if self.mode == 'host':
            print("启动前")
            docker_run = "docker run --network host -itd --rm --name {} -v /home/huaru/chenxiaoyu/ma_transformer_0916_e25/RTMData/bd_train/{}:/home/x64/RTMData -w /home/x64 {} python daemon_server.py {} {} {} {}"\
                .format(self.docker_name, time.time(), self.image_name, self.xsim_time_ratio, self.port, HostID, RTMNum)
            print("启动后")
        else:
            docker_run = "docker run -p {}:{} -itd --rm --name {} -w /home/x64 {} python daemon_server.py {} {} {}  {}"\
    ...
    def close_env(self):
        ...
        if 'Exited' in container_info:
            docker_cmd = 'docker rm -f {}'.format(self.docker_name)
