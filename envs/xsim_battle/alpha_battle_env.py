...
class AlphaBattleEnv():
    ...
    def step(self, model_action):
        ...
        if mode == 2:
            ...
            rewards = np.ones((5, 1), dtype=np.float32)
            dones = np.zeros((5), dtype=bool)

            return state, obs, baseline_state, rewards, dones, infos, level1_action_mask
    ...
    def reset(self):
        ...
        state, obs, baseline_state, availalbe_actions = self.maybe.reset()
        self.enemy_agent.reset()

        return state, obs, baseline_state, availalbe_actions
